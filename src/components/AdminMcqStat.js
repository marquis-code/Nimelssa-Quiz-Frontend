...
import { getAdminMcqQuizStat } from '../apis/auth';
...
const AdminMcqStat = () => {
    const [mcqStat, setMcqStat] = useState([]);
    ...
    useEffect(() => {
        ...
        getAdminMcqQuizStat().then((response) => {
            setMcqStat(response.data)
    ...
    return (
        <>
            <Helmet><title>Nimelssa Quiz | MCQ statistics</title> </Helmet>
            ...
            {mcqStat.length > 0 ? mcqStat.map((singleStat, index) => {
